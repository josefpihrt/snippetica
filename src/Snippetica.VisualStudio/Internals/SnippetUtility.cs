...
{
    /// <summary>
    /// Returns enumerable groups of <see cref="Snippet"/>s that have same shortcut..
    /// </summary>
    ...
    internal static IEnumerable<DuplicateShortcutInfo> FindDuplicateShortcuts(IEnumerable<Snippet> snippets)
    {
        if (snippets is null)
        ...
        IEnumerable<DuplicateShortcutInfo> FindDuplicateShortcuts()
        {
                ...
                .GroupBy(f => f.Snippet, SnippetComparer.Shortcut))
            {
                if (grouping.CountExceeds(1))
                    yield return new DuplicateShortcutInfo(grouping.Key.Shortcut, grouping.Select(f => f.Snippet).ToList());
            }
        }
    }
    ...
    public static void RemoveUnusedLiterals(Snippet snippet)
    {
        ...
        foreach (SnippetLiteral literal in snippet.Literals)
        {
    ...
    public static void RemoveUnusedPlaceholders(Snippet snippet)
    {
        ...
        foreach (SnippetPlaceholder placeholder in snippet.Code.Placeholders)
        {
